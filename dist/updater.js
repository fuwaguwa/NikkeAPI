...
function updateFile(obj) {
    const jsonPath = path_1.default.join(__dirname, "..", "data", "NikkeCharsInfo.json");
    fs_1.default.readFile(jsonPath, "utf-8", (err, data) => {
        const json = JSON.parse(data);
        json.data.push(obj);
        fs_1.default.writeFile(jsonPath, JSON.stringify(json, null, "\t"), "utf-8", (err) => {
            if (err)
                console.error(err);
            console.log("Success!");
        });
...
function resetFile() {
    const jsonPath = path_1.default.join(__dirname, "..", "data", "NikkeCharsInfo.json");
    fs_1.default.writeFile(jsonPath, JSON.stringify({
        data: [],
    }, null, "\t"), "utf-8", (err) => {
        if (err)
            console.error(err);
...
(0, node_fetch_1.default)("https://www.prydwen.gg/page-data/sq/d/1372249733.json")
    ...
    await sleep(500);
    for (let i = 0; i < json.data.allContentfulNikkeCharacter.nodes.length; i++) {
